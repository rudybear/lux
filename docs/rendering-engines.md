...
| `--output-aux <PREFIX>` | Splats compiled with `motion_vectors`/`expected_depth`/`foreground_coverage`: write `<PREFIX>_color.png`, `_color.npy` (float32 `[H,W,4]`, un-premultiplied RGBA read directly off the RGBA16F color attachment, no 8-bit quantization), `_depth.npy`, `_mv.npy` (float32, row-major `[H,W,C]`), and (when `foreground_coverage: true`) `_fg.npy` (float32 `[H,W]`, un-premultiplied by `out_depth.w` off `out_depth`'s `.g` channel — see SPECIFICATION.md 12.8) plus normalized PNG previews |
