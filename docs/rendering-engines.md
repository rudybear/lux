...
| `--output-aux <PREFIX>` | Splats compiled with `motion_vectors`/`expected_depth`/`foreground_coverage`: write `<PREFIX>_color.png`, `_color.npy` (float32 `[H,W,4]`, un-premultiplied RGBA read directly off the RGBA16F color attachment, no 8-bit quantization), `_depth.npy`, `_mv.npy` (float32, row-major `[H,W,C]`), and (when `foreground_coverage: true`) `_fg.npy` (float32 `[H,W]`) plus normalized PNG previews. `_depth`/`_mv` are un-premultiplied off the packed `out_aux` RGBA32F attachment (`x=mv.x*alpha, y=mv.y*alpha, z=depth*alpha, w=alpha` — `.w` is the GENUINE per-fragment alpha, required for correct hardware blend accumulation, not recoverable from `out_color`'s alpha; bench/lux_perf_ablation.md task 2 — replaces the earlier two-attachment `out_motion`/`out_depth` design with zero wasted lanes); `_fg` is un-premultiplied off a SEPARATE, smaller `out_fg` RGBA16F attachment (`x=fg*alpha, w=alpha`) that can't share `out_aux`'s lanes — see SPECIFICATION.md 12.8. The output `.npy` filenames/shapes/semantics are unchanged from before the packing change. |
