...
`playground_ios/` is unaffected today: `SplatView.mm` drives `MetalSplatRenderer` (the hand-written MSL "hand" backend, `_splatRProxy`), whose `getMotionTexture()`/`getExpectedDepthTexture()`/`kExpectedDepthChannels` API is untouched by this change (only `MetalSplatLuxcRenderer`, the luxc-compiled backend, was repacked — see `playground_cpp/src/metal_splat_luxc_renderer.h`'s `getAuxTexture()`/`getFgTexture()`). The two backends' public surfaces are intentionally kept parallel-but-independent (`playground_cpp/src/metal_main.cpp`'s `runSplatBranch<Renderer>` template branches on `if constexpr (std::is_same_v<Renderer, MetalSplatLuxcRenderer>)` to pick the right accessor/layout per backend) — whichever `playground_ios` migration eventually switches `_splatRProxy` from `MetalSplatRenderer` to `MetalSplatLuxcRenderer` (per this section's own intro comment, "switch to `MetalSplatLuxcRenderer` once that path is validated") will need the same `getMotionTexture`/`getExpectedDepthTexture` → `getAuxTexture`/`getFgTexture` migration as `playground_android/` above, at `SplatView.mm`'s lines 705, 710-711, 748, 923, 937.
