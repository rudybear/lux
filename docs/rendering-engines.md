...
**Foreground/actor coverage (`gltf_loader.cpp`, feeds the `foreground_coverage` splat-block flag — SPECIFICATION.md 12.8):** the C++ loader populates `GaussianSplatData::foreground` (one 0.0/1.0 value per splat) from, in order of precedence: (1) a custom `_FOREGROUND` vertex attribute on the splat `POINTS` primitive (`SCALAR`, `UNSIGNED_BYTE`, normalized — 255 means foreground; mobiledlss's dyn-splat writer emits this and records `mesh.extras.MOBILEDLSS_dynamic_splats.foreground_attribute = "_FOREGROUND"`); (2) if absent, the union of `dynamics.targets[*].indices` across all morph-target keyframes — i.e. a gaussian is treated as foreground iff it has *any* morph-target delta at any keyframe, on the theory that only the animated actor moves in a static-background capture; (3) if the primitive has neither an explicit attribute nor any morph targets, all-zero. This is host-side data prep only — the compiler side (`splat_foreground` input buffer, composited into a dedicated `out_fg` attachment — bench/lux_perf_ablation.md task 2 merged the earlier separate `out_motion`/`out_depth` attachments into one packed `out_aux` [mv.xy, depth, alpha], with `out_fg` [foreground, alpha] kept as its own second attachment since `out_aux` has no spare non-alpha lane left) is documented in SPECIFICATION.md 12.8 and `docs/language-reference.md`'s Gaussian Splatting section. `--output-aux <prefix>` writes the composited, un-premultiplied result to `<prefix>_fg.npy` (float32 `[H,W]`) on both the Vulkan and Metal CLIs when `foreground_coverage: true` was compiled in.
...
| `--output-aux <PREFIX>` | Splats compiled with `motion_vectors`/`expected_depth`/`foreground_coverage`: write `<PREFIX>_color.png`, `_color.npy` (float32 `[H,W,4]`, un-premultiplied RGBA read directly off the RGBA16F color attachment, no 8-bit quantization), `_depth.npy`, `_mv.npy` (float32, row-major `[H,W,C]`), and (when `foreground_coverage: true`) `_fg.npy` (float32 `[H,W]`) plus normalized PNG previews. `_depth`/`_mv` are un-premultiplied off the packed `out_aux` attachment (`x=mv.x*alpha, y=mv.y*alpha, z=depth*alpha, w=alpha` — `.w` is the GENUINE per-fragment alpha, required for correct hardware blend accumulation, not recoverable from `out_color`'s alpha; bench/lux_perf_ablation.md task 2 — replaces the earlier two-attachment `out_motion`/`out_depth` design with zero wasted lanes), RGBA16F by default (`aux_precision: half`, `gaussian_splat_dlss.lux`'s default) or RGBA32F if compiled with `aux_precision: float`; `_fg` is un-premultiplied off a SEPARATE, smaller `out_fg` RGBA16F attachment (`x=fg*alpha, w=alpha`, ALWAYS RGBA16F regardless of `aux_precision`) that can't share `out_aux`'s lanes — see SPECIFICATION.md 12.8. The output `.npy` filenames/shapes/semantics are unchanged from before the packing change. |
...
## Android live-rendering demo (playground_android/)

Stage A of the mobile-DLSS live demo, Android side: a NativeActivity app (`playground_android/`, no Gradle -- `build.sh` cross-compiles with the NDK's CMake toolchain file and packages the APK by hand with `aapt2`/`zipalign`/`apksigner`) that reuses the *Vulkan* (not Metal) splat pipeline from `playground_cpp/src` by reference -- `SplatRenderer`, `SceneManager`, `GltfLoader`, `Camera`, `DlssIO`, `SpvLoader`, `Scene` compiled unmodified via relative source paths into `playground_android`'s own CMake target -- to live-render the same pruned juggle scene (`mobiledlss/demo/ios_assets/juggle_p0.8_stride4.glb`) on a Pixel 9 Pro XL (Tensor G4 / Mali-G715, Vulkan 1.4 driver). `playground_cpp/src/vulkan_context.cpp` itself is the one file NOT reused (it `#include`s GLFW, unavailable on Android); `playground_android/src/android_vulkan_context.{h,cpp}` re-implements the same instance/surface/device/swapchain/allocator bring-up against `VK_KHR_android_surface` + `ANativeWindow` instead, filling in the same shared `VulkanContext` struct so the rest of the pipeline is none the wiser. Camera: same orbit formula as the iOS demo (`mobiledlss/datagen/camera.py::orbit_path`/`look_at`, PanopticSports y-down world), but centered on `lux-4dgs/tests/assets/cameras/generate_juggle_camera.py`'s known-good actor center `(-0.22, -1.04, 0.09)` rather than `SceneManager::computeAutoCamera`'s whole-scene (person + ~3-unit dome background) bounding-box centroid -- the latter put the orbit ~15.8 units out, camera outside the entire dome looking at a blurry speck. Runs at 960x540 (`kWidth`/`kHeight` in `android_main.cpp`) at ~12fps unoptimized (same caveats as iOS Stage A: full splat draw + GPU radix sort every frame, no proxy-res/jitter/network/reconstruct pass yet).

**Consumer migration needed (bench/lux_perf_ablation.md task 2, packed `out_aux`/`out_fg` DLSS attachments — NOT applied to this directory, see "do not edit playground_android/" in the task brief; applied instead to the isolated `playground_android_perf/` harness copy as the reference implementation):**
`SplatRenderer::getMotionImage()`/`getExpectedDepthImage()`/`getMotionFormat()`/`getExpectedDepthFormat()` no longer exist — `playground_cpp/src/splat_renderer.h`/`.cpp` now expose `getAuxImage()` (`VkFormat` `R32G32B32A32_SFLOAT`, unchanged precision) carrying `(mv.x*alpha, mv.y*alpha, depth*alpha, alpha)` -- `.w` is the genuine per-fragment alpha (required for correct hardware blend accumulation; do NOT substitute `getOutputImage()`'s color alpha here, that was a real, measured 10-50x-error bug during this task, not a valid shortcut) -- and, only when `hasForegroundCoverage()`, a second `getFgImage()` (`VkFormat` `R16G16B16A16_SFLOAT`) carrying `(fg*alpha, 0, 0, alpha)`. `playground_android/src/android_main.cpp` calls the old accessors at 4 sites and will fail to compile against a `playground_cpp/src` rebuilt at or after this change:
- `dumpProxyDebugFrame()` (~line 663-690): reads `getExpectedDepthImage()`/`getMotionImage()` as two separate `RGBA32F` (16 bytes/pixel) reads, each with its own `.w` alpha. Replace with one `getAuxImage()` read (still `RGBA32F`, 16 bytes/pixel, plain `memcpy` -- no format change here) giving `depth` at `.z` (moved from `.x`) and un-premultiplying by `out_aux`'s own `.w`; `fg` (when `hasForegroundCoverage()`) now needs a SEPARATE `getFgImage()` read (`RGBA16_SFLOAT`, 8 bytes/pixel -- half-float, needs `DlssIO::halfToFloat()` per channel) un-premultiplied by `out_fg`'s own `.w`, not `out_depth`'s old `.y`/`.w`.
- `readMvProxy()` (~line 731): same `getMotionImage()` → `getAuxImage()` change (still `RGBA32F`, plain `memcpy`, un-premultiply by `out_aux.w`), extracting `.xy` only.
- The `state->inputAssembly.run(...)` call (~line 1035-1036): currently passes `getExpectedDepthImage()`/`getMotionImage()` as two separate GLSL-compute-consumed images; `input_assembly_unpremul_depth.comp`/`input_assembly_assemble.comp` (`playground_android/shaders_glsl/`) assume the old two-attachment `RGBA32F` byte layout and channel offsets. The packed layout's `depth` (now `out_aux.z`, was `out_depth.x`) is still `RGBA32F`, only the offset moved; `fg` moved from `out_depth`'s `.y` to a wholly separate `out_fg` attachment (`RGBA16F`) these shaders don't know about at all yet — this needs real Stage-3 shader rework, not just a call-site signature change, before Proxy-mode (`isProxy`) frames can trust the assembled tensor again. `playground_android_perf/src/android_main.cpp`'s equivalent call site (same function, same line shape) documents this same gap inline with a `NOTE` comment — this harness's own `renderFrame()` never runs in `isProxy` mode, so the gap is real but unexercised there too.
- The complete working reference for the two dump-path fixes (correct, tested via a full desktop CLI build, bit-exact vs. the pre-packing baseline) is `playground_android_perf/src/android_main.cpp`'s `dumpProxyDebugFrame()`, and the exactly analogous fix already landed in `playground_cpp/src/main.cpp`'s `--output-aux` path — copy the pattern from either.

**`playground_ios/` IS affected (correction — an earlier version of this note said otherwise, based on stale information that `SplatView.mm` still drove `MetalSplatRenderer`):** as of `80e33f4`, the iOS app drives `MetalSplatLuxcRenderer` for both the Proxy and Target passes, not the hand-written `MetalSplatRenderer` "hand" backend — so it needs the SAME accessor migration as `playground_android/` above: `getMotionTexture()`/`getExpectedDepthTexture()`/`kExpectedDepthChannels` → `getAuxTexture()` (RGBA16Float by default now that `aux_precision: half` is `gaussian_splat_dlss.lux`'s default — RGBA32Float only if compiled with `aux_precision: float` explicitly; check `auxPrecisionHalf()`/`getAuxFormat()` rather than assuming a fixed format) + `getFgTexture()` (RGBA16Float always, only when `hasForegroundCoverage()`) — see `playground_cpp/src/metal_splat_luxc_renderer.h`'s `getAuxTexture()`/`getFgTexture()` comments for the exact packed layout and the measured blend-correctness bug that shaped it. `playground_cpp/src/metal_main.cpp`'s `runSplatBranch<Renderer>` template (`if constexpr (std::is_same_v<Renderer, MetalSplatLuxcRenderer>)`) is the reference implementation for both the `MetalSplatLuxcRenderer` and `MetalSplatRenderer` accessor patterns side by side. `SplatView.mm`'s call sites are at lines 705, 710-711, 748, 923, 937 (same shape as `playground_android/`'s `dumpProxyDebugFrame()`/`readMvProxy()`/`inputAssembly.run()` migration described above — `depth` moves from `.x` to `.z` within `out_aux`, `fg` moves to the separate `out_fg` texture's `.x`, and each texture's own `.w` is genuine alpha, not `out_color`'s).

Toolchain (all via `brew`, no Android Studio): `android-commandlinetools` (cask; provides `sdkmanager`, `aapt2`, `d8`, `apksigner`, `zipalign`) + `sdkmanager "platforms;android-35" "build-tools;35.0.0"`, `openjdk` for the JDK `sdkmanager`/`apksigner` need, and the Homebrew `android-ndk` (r29) already on this Mac for `aarch64-linux-android29-clang++`. No Gradle: `build.sh` runs `cmake -G Ninja` with `-DCMAKE_TOOLCHAIN_FILE=$NDK/build/cmake/android.toolchain.cmake`, then stages `liblux_android.so` (+ `libc++_shared.so`) into `lib/arm64-v8a/`, links resources with `aapt2 link` against a hand-written `AndroidManifest.xml` declaring `android.app.NativeActivity` (`android:hasCode="false"`, no Java/Kotlin at all), zips the native libs in, `zipalign`s, and `apksigner sign`s with a debug keystore generated on first run. `push_assets.sh` stages the luxc-compiled `examples/gaussian_splat_dlss.*` shaders, `shaders/radix_sort/*.spv`, and the scene `.glb` onto the device.

Gotchas worth recording:
- **`vk-bootstrap`'s vendored dispatch table won't compile against the NDK's bundled Vulkan headers.** The NDK r29 sysroot ships Vulkan headers at 1.3.275, but `vk-bootstrap-src`'s `VkBootstrapDispatch.h` (fetched by `playground_cpp`'s desktop CMake build, reused here) references `VK_EXT_depth_clamp_control` types added after that. Fix: `FetchContent` a fresh `KhronosGroup/Vulkan-Headers` checkout and put its `include/` ahead of the NDK sysroot's on the include path for both `vk-bootstrap` and the app target (header-only; doesn't change what's actually linked at runtime).
- **The NDK's `libvulkan.so` stub statically exports only what `ANDROID_PLATFORM` guarantees, not what the on-device driver actually supports.** `vkGetBufferDeviceAddress` (core in 1.2) and VMA's `vkGetDevice{Buffer,Image}MemoryRequirements` (core in 1.3) linked fine on desktop (loader always has them) but came back `undefined symbol` at Android link time even though the Mali-G715 driver is Vulkan 1.4. Fix: `VMA_STATIC_VULKAN_FUNCTIONS=0` + `VMA_DYNAMIC_VULKAN_FUNCTIONS=1` (VMA resolves everything via `vkGetInstanceProcAddr`/`vkGetDeviceProcAddr` at runtime instead of static linking), and the same pattern by hand for `vkGetBufferDeviceAddress` in `VulkanContext::getBufferDeviceAddress()`.
- **`adb push` straight into `/sdcard/Android/data/<pkg>/files` produces files the app's own process can't `fopen()`.** `cgltf_parse_file` came back `cgltf_result_file_not_found` on a file `adb shell ls` showed at the exact right size with `rw-rw-rw-` permissions -- Android 11+ scoped storage enforces per-app isolation on that path below the POSIX permission bits shown to the shell. Fix: stage through `/data/local/tmp` and pipe into the app's *internal* files dir via `run-as <pkg>` (app is `android:debuggable="true"`), i.e. `app->activity->internalDataPath` instead of `externalDataPath`.
- **A stale `android_native_app_glue` poll timeout silently starves the render loop.** The obvious `int timeoutMs = vulkanReady ? 0 : -1;` computed once *before* the `while (ALooper_pollOnce(timeoutMs, ...) >= 0)` loop reads `vulkanReady` from before `APP_CMD_INIT_WINDOW` was processed; once that event's processing flips it true mid-callback, the *same* inner loop iterates again with the stale blocking `-1` and hangs forever in `ALooper_pollOnce` waiting for another input/window event that never comes (no crash, no error -- the process just sits at 0% CPU in state `S`). Fix: recompute the timeout on every inner-loop iteration, not once above it.
- **The camera "center" matters more than matching the elevation/radius formula.** Using `SceneManager::computeAutoCamera`'s auto-framed eye/target (whole scene incl. background) instead of the scene-specific actor center from `generate_juggle_camera.py` produced two successive wrong results before landing on the right one: first a camera *inside* the splat cloud (giant unsorted-looking overlapping ellipses filling the frame), then, after generalizing the orbit basis to the scene's own up/distance, a camera *outside the whole dome* looking at a tiny blurry blob. The fix wasn't a formula bug -- `orbit_path`/`look_at` were correct from the first attempt -- it was reusing the validated fixed actor center `(-0.22, -1.04, 0.09)` + `radius=2.0` + `world_up=(0,-1,0)` from `tests/assets/cameras/generate_juggle_camera.py` instead of deriving a center from the mesh bounding box.

Validated stage: live orbiting render on-device matches the iOS reference screenshot's dome-wall/floor/actor composition and PanopticSports-style splat noise character (`mobiledlss/demo/ios_assets/ipad_stageA_screenshot.png`) at the same 960x540 internal resolution, ~12fps on the Pixel 9 Pro XL's Mali-G715.
