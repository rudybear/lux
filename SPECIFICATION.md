...
**`sort`**: `camera_distance` (default, matching the ratified `KHR_gaussian_splatting` spec's `sortingMethod: cameraDistance`) computes the true Euclidean distance from the camera (`-length(view_pos)`, negated to preserve the ascending-sort-key = back-to-front draw order the fixed-function alpha blend requires); `view_depth` (gsplat's own convention) uses raw view-space `z` instead — added so the two can be measured against each other. Previously `sort` was accepted but never actually affected codegen (always used view-space `z`, i.e. behaved like today's `view_depth` regardless of the declared value) — this is now a real, functioning option. `camera_distance` is the `KHR_gaussian_splatting` spec default; `view_depth` is what the 3DGS reference rasterizer and gsplat use — pick `view_depth` when the scene or a downstream network was trained/validated against those renderers (measured on mobiledlss's `juggle` clip, frame 40, against gsplat: 49.4 dB colour PSNR and exact depth with `view_depth`, vs. only 30.9 dB and 0.8% depth median relative error with `camera_distance` — see `examples/gaussian_splat_dlss.lux`, which sets `sort: view_depth` for exactly this reason).
...
- `motion_vectors: true` adds an input buffer `splat_prev_pos` (previous frame's animated world position, host double-buffered), two preprocess push fields (`proj_matrix_unjittered`, `prev_view_proj_unjittered` — jitter must never leak into the MV), and an output buffer `projected_mv` (vec2, pixels; backward convention `uv_prev = uv - mv`).
- `expected_depth: true` adds an output buffer `projected_depth` (scalar camera-space `z`, i.e. `t` from the existing Jacobian projection).
- When EITHER is set, the fragment stage emits ONE additional attachment `out_aux` (vec4, RGBA32F on the host: `x = frag_mv.x * alpha`, `y = frag_mv.y * alpha`, `z = frag_depth * alpha`, `w = alpha` — lanes for a disabled feature are written `0.0`), blended with the SAME premultiplied-alpha equation/factors as `out_color`. **(bench/lux_perf_ablation.md task 2, current design)**: earlier lux versions used TWO attachments (`out_motion`, `out_depth`, each RGBA32F, each with an always-`0.0` `.z` and its own copy of alpha in `.w`). `out_aux` merges them, packing 3 real values (mv.xy, depth) with ZERO wasted lanes — attachment count 2→1, bytes 32B→16B, whenever `foreground_coverage` is off. `.w` MUST stay the genuine per-fragment alpha (not a repurposed data channel, and NOT recovered from `out_color`'s alpha instead): Vulkan's fixed-function `SRC_ALPHA`/`ONE_MINUS_SRC_ALPHA` blend factors read "source alpha" from THIS attachment's own 4th output component specifically, so anything else there corrupts the GPU-side blend accumulation itself, not merely host-side precision — an earlier version of this design packed `foreground*alpha` into `.w` instead (reasoning the host could divide by `out_color`'s alpha afterward), which produced a measured 10-50x MV/depth error in heavily-overdrawn background regions (`ONE_MINUS_SRC_ALPHA` evaluating to ~1, i.e. no decay, for every non-actor fragment — an unbounded running sum instead of a proper "over" composite). `out_aux` also stays RGBA32F, not RGBA16F: downgrading it to half-float reproduces the same class of catastrophic error even with the alpha bug fixed, since premultiplied mv/depth values (unlike `out_color`'s own [0,1]-bounded channels) are not magnitude-bounded and compound half-float blend rounding badly over many overlapping low-alpha splats.
- Reflection's `gaussian_splatting` section reports both `motion_vectors`/`expected_depth` booleans, independent of the merged `out_aux` output. Jitter (`--jitter`), previous-frame buffering, and headless aux dumps (`--output-aux`) / camera bridge (`--camera-json`) are playground (host) responsibilities — see `docs/lux-4d-spec.md` sections 3-4.
- **`aux_precision: float | half`** (optional, **default `half`**): a host-format hint for `out_aux` ONLY, reflected into `gaussian_splatting.aux_precision` — does NOT change the compiled SPIR-V at all (`out_aux` is the identical `vec4` fragment output above either way); only which `VkFormat`/`MTLPixelFormat` the host creates the `out_aux` attachment as changes (`SplatRenderer`/`MetalSplatLuxcRenderer`'s `getAuxFormat()`). `half` (default) is `out_aux` RGBA16F (8B/px) — MEASURED (`examples/gaussian_splat_dlss_half.lux`, `bench/lux_perf_ablation.md`) to fail this repo's original, pixel-exact bar (MV median error 0.012–0.016px vs. a ≤0.01px bar, depth relative median error ~5.4e-3 vs. a ≤1e-3 bar), but the MODEL-LEVEL verdict — the actual mobiledlss DLSS reconstructor trained/evaluated on lux-rendered clips, not a pixel-exact `.npy` diff — found it indistinguishable from `float`: 35.494 vs. 35.491 dB PSNR, `fg` bit-exact, MV 0.008px, depth 0.4%, all invisible to the reconstructor (which only ever consumes `out_aux`/`out_fg` through further lossy stages — bilinear resampling, a learned network — with far more slack than a pixel-exact comparison allows). `float` is `out_aux` RGBA32F (16B/px) — the original pixel-precise design from task 2, still selectable for callers that need the tighter bar (e.g. a non-network downstream consumer reading `out_aux` directly). `out_fg` is DELIBERATELY NOT affected by this option — it stays RGBA16F regardless of `aux_precision` (see the `foreground_coverage` note below): an earlier attempt also shrank `out_fg` to RG16F under `half`, which broke the fixed-function blend outright (a 2-channel format has no `.w` for `SRC_ALPHA`/`ONE_MINUS_SRC_ALPHA` to read, so it used a constant alpha instead — a full 0-to-1 flip on ~1% of pixels, a correctness bug, not a precision tradeoff) — that fix is what makes `half` safe to default to at all.

**`foreground_coverage: true`** (optional, default `false`; implies `expected_depth: true` even when not set explicitly — there is nowhere else to write it without that attachment): a per-pixel visibility-weighted composite of a per-splat is-foreground flag, for reconstruction networks (e.g. mobiledlss) that need an actor/foreground mask alongside colour/MV/depth. A SECOND attachment `out_fg` (vec4, RGBA16F on the host: `x = frag_foreground * alpha`, `y`/`z` unused, `w = alpha`) — can't share `out_aux`'s lanes, since `out_aux` already uses all 3 non-alpha lanes for mv.xy/depth and `.w` must independently stay genuine alpha in EVERY blended attachment (see the `out_aux` note above). `out_fg` is safely RGBA16F (not RGBA32F): `foreground` is bounded to `[0, 1]` like alpha itself, so it doesn't suffer the unbounded-magnitude precision problem `out_aux`'s mv/depth values do (measured: no precision regression vs. RGBA32F on the reference scene). `out_fg`'s format is ALWAYS RGBA16F, independent of `aux_precision` above — it does not shrink further under `aux_precision: half` (see that note for why a 2-channel `out_fg` is unsafe, not merely lower-precision). Adds an input buffer `splat_foreground` (scalar, 0.0/1.0 per splat — host-populated, see `docs/rendering-engines.md`'s glTF-loader section for the `_FOREGROUND`-attribute/morph-delta-fallback/all-zero precedence) and an output buffer `projected_foreground` (scalar, straight per-splat passthrough — the flag doesn't vary per-frame, so the preprocess stage does no computation on it beyond the buffer-to-buffer copy) plus a `frag_foreground` vertex-to-fragment varying. Adds no push-constant fields. The host divides `out_fg.x` by `out_fg.w` to recover the coverage value in `[0, 1]`.

### 12.9 Reconstruction Pass (DLSS-style upscale/accumulate)

A `reconstruct { s: 2, k: 4, param_stride: 1, hidden: 8 }` block, referenced by a `pipeline { mode: reconstruct, reconstruct: X }`, is a runtime port of `mobiledlss.train.reconstruct` (see `docs/lux-reconstruct-spec.md` in the mobiledlss repo). It emits three independent compute stages — `warp`, `apply`, `blend` (`luxc/expansion/reconstruct_expander.py`), each a single flat 1-D dispatch over the `target_w * target_h` output pixels (`global_invocation_id.x` as a linear thread id, workgroup size 256, matching the splat preprocess stage's convention), decomposed to `(X, Y)` via integer div/mod:

- **`s`, `k`, `param_stride`, `hidden`** are baked in as compile-time literals (one compiled pipeline == one fixed network architecture, exactly like a splat block's `sh_degree`); `sp = s * param_stride` is the un-multiplex factor `pixel_shuffle_params` uses. Resolutions (`target_w/h`, `proxy_w/h`, `net_w/h`) and the per-frame `jitter_x/jitter_y` are runtime push-constant fields, since one compiled pipeline is reused across every clip/frame resolution.
- **`warp`** (`mobiledlss.train.reconstruct.warp`, colour history only — stage 1 of this port has no network in the loop, see below): storage buffers `prev_color` (in), `mv_proxy` (in, full proxy resolution, nearest-upsampled by `s` and scaled), `warped_color` (out) — all flat `scalar` arrays, `[H, W, C]` row-major, no padding. 4-tap bilinear gather at the pixel-centre sample position, zero-padded outside `[0, size)` (not clamped, not renormalised) — `grid_sample(..., padding_mode="zeros", align_corners=False)`'s exact semantics.
- **`apply`** (un-multiplex + apply-kernel fused, `pixel_shuffle_params` + `apply_kernel`): storage buffers `packed_params` (in, network resolution, channel layout `[kernel_logits (sp²·K², channel = kk·sp² + dy·sp + dx), blend_logits (sp²), hidden_raw (hidden)]` — `ParamPredUNet.forward_packed`'s own layout, no re-packing needed), `proxy_color` (in), `spatial_out`/`alpha_out`/`hidden_out` (out). Softmax over the `K²` kernel taps (numerically stable, max-subtracted) and sigmoid of the one blend-logit channel for this block offset (2-way blend only — a `blend_logits` channel count `> 1`, i.e. a scene-memory 3-way blend, is out of scope for this port); `hidden_out` is a **plain broadcast copy** of `packed_params`'s hidden channels (nearest-upsample, *not* pixel-shuffled, matching `pixel_shuffle_params`'s own `F.interpolate(..., mode="nearest")` treatment of `hidden_raw`). Kernel taps are gathered from `proxy_color` via `_tap_offsets(K)` around `round(cx), round(cy)` (jitter-corrected proxy coordinate), each an exact **clamped-to-border integer index** — `apply_kernel`'s own `mode="nearest"` degenerates to this since its grid-sample coordinate is always exactly integer-valued; the *weights* (softmax kernel) supply all the sub-pixel blending.
- **`blend`** (`mobiledlss.train.reconstruct.blend`, 2-way): storage buffers `spatial_out`, `warped_color`, `alpha_out`, `disocc` (all in), `out_color` (out). `alpha_eff = disocc > 0.5 ? 1 : alpha`; `out = lerp(warped_color, spatial_out, alpha_eff)`.
- **Not ported** (stage 2, `docs/lux-reconstruct-spec.md`'s own scope note): network execution (packed params are supplied by the host, computed offline or — once stage 2 lands — by a `network` block), proxy-depth-based disocclusion, Halton jitter driving the splat projection matrix, and the `hidden_in` warp+downsample a *live* per-frame network call would need (this port's `hidden_out` is a pure per-frame function of `packed_params`, since no network runs between frames here).

**Scene-memory path** (`memory: { channels, hidden }` sub-block, `docs/scene-memory-spec.md` in the mobiledlss repo — `SceneTexture` + `MemoryColorHead`): when present, `apply` predicts a 3-way blend and two more stages are emitted:

- **`bguv`**: per-pixel sphere UV (`mobiledlss.datagen.camera.sphere_uv`) — far intersection of the pixel's world-space view ray with a bounding sphere (`bg_sphere`, `vec4`: centre.xyz, radius); `u = atan2(d.z, d.x)/2π + 0.5`, `v = acos(clamp(d.y,-1,1))/π`. Storage buffer `bguv_out` (out, `[H,W,2]`). Push constants: `width`, `height`, `k_params` (`vec4`: fx,fy,cx,cy), `cam_to_world` (`mat4`), `bg_sphere` (`vec4`) — fully resolution/camera-generic, so the host runs the identical compiled stage at both proxy and target resolution.
- **`memory`**: bilinear-samples the `channels × tex_h × tex_w` feature texture (`bg_texture`, `[C,H,W]` row-major) at `bguv_in`'s UV — `u` wraps (`fract`), `v` clamps — then decodes through `MemoryColorHead`'s `channels → hidden → 3` MLP (`mem_fc1_w/b`, `mem_fc2_w/b`; `LeakyReLU(0.1)` implemented as `max(x, 0.1x)`, then sigmoid). Writes `bg_features_out` (raw sampled feature, pre-decode — proxy-resolution invocations feed `--dump-bg-features`) and `memory_color_out` (decoded RGB — target-resolution invocations feed `blend`). `channels`/`hidden` are compile-time; the whole per-pixel computation is fully unrolled.
- **`apply`** (3-way): `blend_logits` channel count `3·sp²` (vs. `sp²` for 2-way) selects this path purely by shape, matching `pixel_shuffle_params`; softmax over `{spatial, history, memory}` (channel = `kk·sp² + block_off`, `kk∈{0,1,2}`) replaces the single sigmoid alpha, written to a 3-channel `blend_out` buffer instead of `alpha_out`.
- **`blend`** (3-way, `mobiledlss.train.reconstruct.blend3`/`renormalized_blend_weights`): storage buffers `spatial_out`, `warped_color`, `blend_out` (3ch), `disocc`, `memory_color` (all in), `out_color` (out). History share zeroed where `disocc > 0.5`, the three (already-softmaxed) weights renormalised (`/max(sum, 1e-8)`), `out = w_s·spatial + w_h·warped + w_m·memory`.

The network's own proxy-resolution feature sampling (extra input-channel block) still runs outside this pass; `--dump-bg-features <dir>` runs `bguv`+`memory` standalone at proxy resolution for the network host to consume.

Both playgrounds run these stages standalone from a directory of dumped `.npy` inputs via `--reconstruct-dump <dir>` (`playground_cpp/src/reconstruct_runner.cpp` / `metal_reconstruct_runner.cpp`, the latter transpiling the identical compiled SPIR-V to MSL via the existing `ShaderTranspiler` — no hand-written Metal compute for this pipeline, unlike the splat renderer), writing `out_f{t}.npy` / `hidden_f{t}.npy` per frame, `prev_color`/`prev_hidden` state double-buffered host-side between frames; when the compiled pipeline has the memory path, the dump directory additionally carries `bg_sphere.npy`, `texture.npy`, `memory_head.npz`, and per-frame camera fields, and `out_f{t}.npy` reflects the memory path. See `docs/rendering-engines.md`'s CLI table.
