...
- **`blend`** (`mobiledlss.train.reconstruct.blend`, 2-way only): storage buffers `spatial_out`, `warped_color`, `alpha_out`, `disocc` (all in), `out_color` (out). `alpha_eff = disocc > 0.5 ? 1 : alpha`; `out = lerp(warped_color, spatial_out, alpha_eff)`.
- **Not ported** (stage 2, `docs/lux-reconstruct-spec.md`'s own scope note): network execution (packed params are supplied by the host, computed offline or — once stage 2 lands — by a `network` block), proxy-depth-based disocclusion, Halton jitter driving the splat projection matrix, 3-way scene-memory blending, and the `hidden_in` warp+downsample a *live* per-frame network call would need (this port's `hidden_out` is a pure per-frame function of `packed_params`, since no network runs between frames here).

Both playgrounds run these stages standalone from a directory of dumped `.npy` inputs via `--reconstruct-dump <dir>` (`playground_cpp/src/reconstruct_runner.cpp` / `metal_reconstruct_runner.cpp`, the latter transpiling the identical compiled SPIR-V to MSL via the existing `ShaderTranspiler` — no hand-written Metal compute for this pipeline, unlike the splat renderer), writing `out_f{t}.npy` / `hidden_f{t}.npy` per frame, `prev_color`/`prev_hidden` state double-buffered host-side between frames. See `docs/rendering-engines.md`'s CLI table.
