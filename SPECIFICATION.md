...
- `expected_depth: true` adds an output buffer `projected_depth` (scalar camera-space `z`, i.e. `t` from the existing Jacobian projection) and a fragment output `out_depth` (vec4: `x = frag_depth * alpha`, `y`/`z` unused, `w = alpha`). `vec4`, not `vec2`, is required: Vulkan's fixed-function `ONE_MINUS_SRC_ALPHA` blend factor reads "source alpha" from the 4th component of the fragment output for that attachment, which a `vec2` output doesn't have — on MoltenVK/Apple GPUs this silently produced an undecayed running sum instead of the correct back-to-front "over" composite for overlapping splats. `out_motion` (already vec4) never had this bug.
...
**`foreground_coverage: true`** (optional, default `false`; implies `expected_depth: true` even when not set explicitly — there is nowhere else to write it without that attachment): a per-pixel visibility-weighted composite of a per-splat is-foreground flag, for reconstruction networks (e.g. mobiledlss) that need an actor/foreground mask alongside colour/MV/depth. **No new attachment**: the composited value is packed into `out_depth`'s otherwise-unused `.g` channel (`y = frag_foreground * alpha`, alongside the existing `x = frag_depth * alpha` and `w = alpha`), since `expected_depth`'s `vec4` already has spare lanes and mobile GPUs have limited attachment budgets. Adds an input buffer `splat_foreground` (scalar, 0.0/1.0 per splat — host-populated, see `docs/rendering-engines.md`'s glTF-loader section for the `_FOREGROUND`-attribute/morph-delta-fallback/all-zero precedence) and an output buffer `projected_foreground` (scalar, straight per-splat passthrough — the flag doesn't vary per-frame, so the preprocess stage does no computation on it beyond the buffer-to-buffer copy) plus a `frag_foreground` vertex-to-fragment varying. Adds no push-constant fields. The host un-premultiplies `out_depth.g` by `out_depth.w` (the same alpha already used to un-premultiply `.x`) to recover the coverage value in `[0, 1]`.
