...
- **`aux_precision: float | half`** (optional, default `float`): a host-format hint for `out_aux` ONLY, reflected into `gaussian_splatting.aux_precision` — does NOT change the compiled SPIR-V at all (`out_aux` is the identical `vec4` fragment output above either way); only which `VkFormat`/`MTLPixelFormat` the host creates the `out_aux` attachment as changes (`SplatRenderer`/`MetalSplatLuxcRenderer`'s `getAuxFormat()`). `float` is `out_aux` RGBA32F (16B/px, the design above). `half` is `out_aux` RGBA16F (8B/px) — MEASURED (`examples/gaussian_splat_dlss_half.lux`, `bench/lux_perf_ablation.md`) to **fail** the DLSS-consumer parity gate on both backends: MV median error 0.012–0.016px (gate ≤0.01px), depth relative median error ~5.4e-3 (gate ≤1e-3). Kept as a real, working, **opt-in-only** option — not the default. `out_fg` is DELIBERATELY NOT affected by this option — it stays RGBA16F regardless of `aux_precision` (see the `foreground_coverage` note below): an earlier attempt also shrank `out_fg` to RG16F under `half`, which broke the fixed-function blend outright (a 2-channel format has no `.w` for `SRC_ALPHA`/`ONE_MINUS_SRC_ALPHA` to read, so it used a constant alpha instead — a full 0-to-1 flip on ~1% of pixels, a correctness bug, not a precision tradeoff).
