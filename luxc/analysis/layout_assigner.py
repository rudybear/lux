...
def assign_layouts(module: Module, webgpu: bool = False) -> None:
    ...
    # Compute: standalone, set 0
    compute_stages = [s for s in module.stages if s.stage_type == "compute"]
...
def _assign_stage_layouts(stage: StageBlock, binding_start: int) -> None:
    ...
    # Auto-assign storage buffer bindings
    for sb in getattr(stage, 'storage_buffers', []):
        if sb.set_number is None:
            sb.set_number = set_num
        if sb.binding is None:
            sb.binding = binding
            binding += 1
