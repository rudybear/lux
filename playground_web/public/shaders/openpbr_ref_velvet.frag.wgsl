...
fn main_1() {
    ...
    let _e69 = max(0.8f, 0.045f);
    let _e70 = (_e69 * _e69);
    let _e71 = (_e70 * _e70);
    let _e75 = (((_e62 * _e62) * (_e71 - 1f)) + 1f);
    let _e82 = (((_e69 * _e69) * _e69) * _e69);
    let _e83 = (1f - _e82);
    let _e97 = (vec3<f32>(0.02f, 0.02f, 0.02f) * 1f);
    ...
    if (0f > 0f) {
        let _e275 = local_1;
        local_1 = mix(_e275, ((vec3<f32>(0.02f, 0.02f, 0.02f) * exp(((log(vec3<f32>(1f, 1f, 1f)) / vec3(max(0f, 0.001f))) * 0f))) * 0f), vec3(0f));
    }
    let _e278 = local_1;
    local_1 = (_e278 + vec3<f32>(0f, 0f, 0f));
    if (0f > 0f) {
        let _e283 = ((1.5f - 1f) / (1.5f + 1f));
        let _e291 = ((1.6f - 1f) / (1.6f + 1f));
        let _e292 = (_e291 * _e291);
        let _e305 = clamp(_e265, 0f, 1f);
        let _e309 = ((1f - (_e305 * _e305)) / (1.6f * 1.6f));
        let _e312 = sqrt((1f - min(_e309, 0.9999f)));
        let _e313 = (1.6f * _e312);
        let _e316 = ((_e305 - _e313) / (_e305 + _e313));
        let _e317 = (1.6f * _e305);
        let _e320 = ((_e317 - _e312) / (_e317 + _e312));
        let _e327 = mix(mix((0.5f * ((_e316 * _e316) + (_e320 * _e320))), 1f, step(1f, _e309)), (1f - ((1f - (_e292 + ((1f - _e292) * ((0.0477f + (0.8965f * _e292)) + ((0.0582f * _e292) * _e292))))) / (1.6f * 1.6f))), 0.8f);
        ...
        let _e386 = ((1f - (_e382 * _e382)) / (1.6f * 1.6f));
        let _e389 = sqrt((1f - min(_e386, 0.9999f)));
        let _e390 = (1.6f * _e389);
        let _e393 = ((_e382 - _e390) / (_e382 + _e390));
        let _e394 = (1.6f * _e382);
        let _e397 = ((_e394 - _e389) / (_e394 + _e389));
        let _e410 = ((1.6f - 1f) / (1.6f + 1f));
        let _e412 = (0f * 0f);
        let _e427 = local_1;
        local_1 = (((((_e427 * mix(vec3<f32>(1f, 1f, 1f), (vec3(max((1f - _e327), 0f)) / max((vec3<f32>(1f, 1f, 1f) - ((mix((vec3<f32>(0.02f, 0.02f, 0.02f) * 1f), vec3<f32>(1f, 1f, 1f), vec3(0f)) * (_e283 * _e283)) * _e327)), vec3<f32>(0.001f, 0.001f, 0.001f))), vec3((0f * 1f)))) * mix(vec3<f32>(1f, 1f, 1f), pow(vec3<f32>(1f, 1f, 1f), vec3((1f / max(_e265, 0.01f)))), vec3(0f))) * (1f - (0f * clamp((((_e410 * _e410) * (1f - ((_e412 * max((1f - _e265), _e265)) * 0.31830987f))) + ((_e412 * max(0f, (0.75f - _e265))) * 0.31830987f)), 0f, 1f)))) + vec3(((((0f * ((_e356 * 0.31830987f) / ((_e360 * _e360) + 0.00001f))) * (0.5f / (((_e351 * sqrt((((_e349 * _e349) * _e368) + _e367))) + (_e349 * sqrt((((_e351 * _e351) * _e368) + _e367)))) + 0.00001f))) * mix((0.5f * ((_e393 * _e393) + (_e397 * _e397))), 1f, step(1f, _e386))) * _e351))) + vec3<f32>(0f, 0f, 0f));
    }
    if (1f > 0f) {
        ...
        let _e471 = local_1;
        local_1 = ((_e471 * (1f - ((1f * max(vec3<f32>(0.4f, 0.4f, 0.4f).x, max(vec3<f32>(0.4f, 0.4f, 0.4f).y, vec3<f32>(0.4f, 0.4f, 0.4f).z))) * 0.3f))) + ((((vec3<f32>(0.4f, 0.4f, 0.4f) * (((2f + _e445) * pow(max((1f - (_e438 * _e438)), 0f), (_e445 * 0.5f))) / 6.2831855f)) * (1f / ((4f * ((_e440 + _e442) - (_e440 * _e442))) + 0.00001f))) * max(_e440, 0f)) * 1f));
    }
    if (0f > 0f) {
        let _e485 = local_1;
        local_1 = (_e485 + (((vec3<f32>(1f, 1f, 1f) * 0f) * 0.01f) * mix(vec3<f32>(1f, 1f, 1f), pow(vec3<f32>(1f, 1f, 1f), vec3((1f / max(_e265, 0.01f)))), vec3(0f))));
    }
    let _e488 = local_1;
