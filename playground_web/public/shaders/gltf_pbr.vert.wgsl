...
fn main_1() {
    ...
    let _e21 = position_1;
    let _e26 = (_e20 * vec4<f32>(_e21.x, _e21.y, _e21.z, 1f));
    world_pos = _e26.xyz;
    ...
    let _e40 = tangent_1;
    let _e41 = _e40.xyz;
    let _e48 = normalize((_e39 * vec4<f32>(_e41.x, _e41.y, _e41.z, 0f)).xyz);
    world_tangent = _e48;
    let _e49 = world_normal;
    let _e51 = tangent_1;
    world_bitangent = (cross(_e49, _e48) * _e51.w);
    let _e54 = uv_1;
    frag_uv = _e54;
    let _e56 = global.projection;
    let _e58 = global.view;
    unnamed.gl_Position = ((_e56 * _e58) * _e26);
    return;
}
...
fn main(@location(0) position: vec3<f32>, @location(1) normal: vec3<f32>, @location(2) uv: vec2<f32>, @location(3) tangent: vec4<f32>, @builtin(vertex_index) vertex_index: u32, @builtin(instance_index) instance_index: u32) -> VertexOutput {
    ...
    let _e24 = frag_uv;
    let _e25 = world_tangent;
    let _e26 = world_bitangent;
