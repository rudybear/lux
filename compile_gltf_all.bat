...
echo [1/15] Compiling hand-written Forward (gltf_pbr.lux)
python -m luxc examples/gltf_pbr.lux -o shadercache/
echo.

REM --- Hand-written RT ---
echo [2/15] Compiling hand-written RT (gltf_pbr_rt.lux)
python -m luxc examples/gltf_pbr_rt.lux -o shadercache/
echo.

REM --- Layered Forward: base (normal_map + emission) ---
echo [3/15] Compiling layered Forward (normal_map + emission)
...
echo [4/15] Compiling layered RT (normal_map + emission)
...
echo [5/15] Compiling layered Forward (normal_map + emission + clearcoat)
...
echo [6/15] Compiling layered Forward (normal_map + emission + sheen)
...
echo [7/15] Compiling layered Forward (normal_map + emission + transmission)
...
echo [8/15] Compiling layered Forward (all extensions)
...
echo [9/15] Compiling layered RT (normal_map + emission + clearcoat)
...
echo [10/15] Compiling layered RT (all extensions)
...
echo [11/15] Compiling layered Forward for SheenChair (normal_map + sheen)
python -m luxc examples/gltf_pbr_layered.lux --pipeline GltfForward --features has_normal_map,has_sheen -o shadercache/
echo.

echo [12/15] Compiling layered RT for SheenChair (normal_map + sheen)
python -m luxc examples/gltf_pbr_layered.lux --pipeline GltfRT --features has_normal_map,has_sheen -o shadercache/ --no-validate
echo.

echo [13/15] Compiling layered Forward for TransmissionTest (emission + transmission)
...
echo [14/15] Compiling layered Forward (clearcoat + normal_map)
...
echo [15/15] Compiling layered Forward (clearcoat + normal_map + sheen)
...
if exist playground_cpp\build\Release\lux-playground.exe (
    ...
    if exist assets\ClearCoatTest.glb (
        ...
        playground_cpp\build\Release\lux-playground.exe --scene assets/SheenChair.glb --pipeline shadercache/gltf_pbr_layered+normal_map+sheen --mode rt --ibl pisa --output screenshots/test_sheen_rt_cpp.png --width 512 --height 512
...
if exist playground_rust\target\release\lux-playground.exe (
    ...
    if exist assets\ClearCoatTest.glb (
        ...
        playground_rust\target\release\lux-playground.exe --scene assets/SheenChair.glb --pipeline shadercache/gltf_pbr_layered+normal_map+sheen --ibl pisa --output screenshots/test_sheen_rust.png --width 512 --height 512
        playground_rust\target\release\lux-playground.exe --scene assets/SheenChair.glb --pipeline shadercache/gltf_pbr_layered+normal_map+sheen --mode rt --ibl pisa --output screenshots/test_sheen_rt_rust.png --width 512 --height 512
