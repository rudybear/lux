...
// --- Shared derivations (see net_input_assembly.mm for the full write-up):
...
// index - raw_mv, Metal coord::pixel sampling with address::clamp_to_zero
// at (c+0.5).
...
kernel void warp_downsample_hidden(
    texture2d<float, access::read> proxyMotion [[texture(0)]],   // RGBA32Float: mv.x*a, mv.y*a, depth*a, a (only .xy/.w read here)
...
kernel void reconstruct_frame(
    ...
    texture2d<float, access::read> proxyMotion [[texture(1)]],   // RGBA32Float: mv.x*a, mv.y*a, depth*a, a (only .xy/.w read here)
...
static MTL::ComputePipelineState* buildPipeline(MetalContext& ctx, const char* src, const char* fnName) {
    ...
    opts->release();
    if (!lib) {
        std::string msg = std::string("LiveReconstructPass: failed to compile ") + fnName;
...
void LiveReconstructPass::init(MetalContext& ctx, const std::string& textureNpyPath,
...
void LiveReconstructPass::prepareHiddenInput(MetalContext& ctx, MTL::CommandBuffer* cmdBuf,
                                              MTL::Texture* proxyMotionTex, bool firstFrame,
...
void LiveReconstructPass::run(MetalContext& ctx, MTL::CommandBuffer* cmdBuf, MTL::Texture* proxyColorTex,
                               MTL::Texture* proxyMotionTex, MTL::Texture* curDepthTex, MTL::Texture* prevDepthTex,
                               bool firstFrame, MTL::Buffer* unetOutputBuffer, MTL::Texture* outColorTex,
                               glm::vec3 eye, glm::vec3 rAxis, glm::vec3 uAxis, glm::vec3 fAxis, float fx, float fy,
                               float cx, float cy, float jitterTargetX, float jitterTargetY) {
    (void)ctx;
    struct ReconUniforms {
