...
struct ImageMemoryBarrier
{
	...
	// LUX-VARIANT: Overload that takes a raw VkImage for non-Texture barriers (e.g. render target transition)
...
#if defined(ENABLE_BENCHMARK)
	void setBenchmarkFrames(uint32_t n) override { m_benchmarkFrames = n; }
#endif
...
private:
	...
	VkPipeline createGraphicsPipeline(VkRenderPass renderPass, uint32_t subpass,
	...
	// LUX-VARIANT: Two separate render passes instead of subpasses
	VkRenderPass m_mainRenderPass;
	VkRenderPass m_tonemapRenderPass;
	...
	// LUX-VARIANT: Sampler for tonemap pass (replaces input attachment)
	...
	// LUX-VARIANT: Separate framebuffers for main and tonemap passes
	std::vector<VkFramebuffer> m_mainFramebuffers;
	std::vector<VkFramebuffer> m_tonemapFramebuffers;
	std::vector<VkCommandBuffer> m_commandBuffers;
	std::vector<VkFence> m_submitFences;
	std::vector<RenderTarget> m_renderTargets;
	std::vector<RenderTarget> m_resolveRenderTargets;
	// LUX-VARIANT: Image views for reading color attachment as sampled image in tonemap
	std::vector<VkImageView> m_tonemapColorViews;
	...
	static constexpr uint32_t NumTimestamps = 5;
	VkQueryPool m_timestampPool = VK_NULL_HANDLE;
	uint32_t m_benchmarkFrames = 0;
